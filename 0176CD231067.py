...
'''
Name: HARSH KUMAR
Enrollment:0176CD231067
Batch: Batch - 5 (MTF)-Batch 2027
Batch Time: 10:30 AM -12:10 PM
'''
...
Obese (30+).
10. Write a program to display the smallest number using nested if.
...
unit=int(input("enter thhe units of electricty"))
if unit<=100:
...
print(f"The smaller number is f>e")
...
if year % 100 == 0:
    ...
    if year % 4 == 0:
        print("However, it is a Leap Year.")
    else:
        print("And it is NOT a Leap Year.")
...
if reverse < 2:
    ...
    for i in range(2, reverse // 2 + 1):
        if reverse % i == 0:

            is_prime = False
            break
if is_prime:
    print(f"{reverse} is a Prime number.")
else:
    print(f"{reverse} is NOT a Prime number.")
...
while total_digit_sum <= 100:
    ...
    print(f"Sum of digits in this number: {digit_sum}")
    print(f"Total digit sum so far: {total_digit_sum}")

print("Digit sum exceeded 100. Program stopped.")
...
while True:
    ...
    if text == text[::-1]:
        print(f"'{text}' is a palindrome. Program stopped.")
        break
    else:
...
''' Write a program using a while loop to compute the sum of digits of a number until the result becomes a
single-digit number (Digital root). Example: 9875 => 9+8+7+5=29 => 2+9=11 => 1+1=2.'''
...
print(f"Twin primes between {start} and {end} are:")
...
print("The sum of the sequence 12 + 22 + 32 + ... + n2")
